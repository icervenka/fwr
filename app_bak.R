...
# change to chron date implementation
...
server <- shinyServer(function(input, output) {
  ...
  resultInput <- reactive({
    ...
    phase <- date_to_phase_arr(date_time, 18, 12)
    #print(phase)
    ...
    tm <- data.frame(matrix(NA, nrow = globalValues$records_count, ncol = 2))
    names(tm) <- c("date_time", "phase")
    tm$date_time <- aligned_time_series2(first_phase_change_dt, start_date2, end_date2, time_interval_min, globalValues$records_count, init_phase_count)
    ...
    result <- gen_agg_parameter(dat_zoo, init_crop, intervals, globalValues$records_count)
  ...
  output$timePlot <- renderPlot({
    result <- resultInput()
    result_cum <- Map(cumsum, result)

    indd <- match(as.numeric(input$timeAggregation), globalValues$com_fact)

    if(input$cumulative == FALSE) {
      plot_data <- data.frame(result[[indd]], check.names = FALSE)
    } else {
      plot_data <- data.frame(result_cum[[indd]], check.names = FALSE)
    }

    plot_data <- add_id_time(plot_data)
    plot_data_melt <- melt(plot_data, id.vars = c("id", "time"))
  
    #print(date_to_phase_arr(plot_data$time, night_start = 18, night_duration = 12))
    #print(plot_data$time)
    
    #print(plot_data$time)
    rect_start <- first_n_ocurrence(date_to_phase_arr(plot_data$time, night_start = 18, night_duration = 12)) + 1
    #print(rect_start)
  ...
  output$groupTimePlot <- renderPlot({
    ...
    isolate({
      ...
      if(group_count > 0) {
        ...
        rects <- dark_plot_rect(rect_start, dim(plot_df)[1], (12/globalValues$com_fact[indd]))
    
        #print(rects)
        plot_df_melt <- melt(plot_df, id.vars = c("id", "time"))

        ggplot() %+% plot_df_melt + aes(x = id, y = value, group = variable, color = variable, linetype = variable) +
          dark_rect_aes(rects) +
          scale_x_continuous(breaks = pretty(1:dim(plot_df)[1], n = 20)) + geom_line(size=0.75)  + graph_options + palette + xlab("interval")
  ...
  output$summaryPlot <- renderPlot({
    ...
    result <- resultInput()
    result_dark_light <- add_phase(result[[6]]) %>%
      summarize_df(c("phase", "variable"), sum_func)

    graph_options <- list(xlab(""), ylab("distance [m]"), theme(axis.text.x = element_text(size=8)))

    grobs <- vector("list", 2*length(sum_func))

    for(i in sum_func) {
      ind <- which(sum_func == i)*2
      grobs[[ind-1]] <- ggplot(result_dark_light) + aes_string("variable", i) + geom_bar(stat="identity") + facet_wrap(~phase, scales = "free") + graph_options + ylab(paste(i, "distance [m]"))
      grobs[[ind]] <- select(result_dark_light, phase, variable, which(colnames(result_dark_light) == i)) %>%
        ggplot() + aes_string("variable", i, fill = "phase") + geom_bar(stat="identity", position=position_dodge()) + graph_options + ylab(paste(i, "distance [m]"))
    }
  ...
  output$groupSummaryPlot <- renderPlot({
    ...
    if(group_count > 0) {
      ...
      plot_df <- group_data(plot_data, columns)
      names(plot_df) <- name_groups(length(columns))
      plot_df <- summarize_df(cbind.data.frame(plot_df, phase = vapply(rownames(plot_df), date_to_phase_par, character(1))), c("phase", "variable"), sum_func)

      grobs <- vector("list", 2*length(sum_func))

      for(i in sum_func) {
        ind <- which(sum_func == i)*2
        grobs[[ind-1]] <- ggplot(plot_df) + aes_string("variable", i) + geom_bar(stat="identity") + facet_wrap(~phase, scales = "free") + graph_options + ylab(paste(i, "distance [m]"))
  ...
  output$hourPlot <- renderPlot({
    result_hour <- resultInput()[[1]]

    dat_hour <- cbind.data.frame(result_hour, hour = as.integer(format(time(result_hour), "%H")))
    dat_hour_sum <- melt(dat_hour, id.vars = "hour") %>%
      group_by(hour, variable) %>%
      summarize(sum = sum(value))

    graph_options <- c(graph_options, geom_line(size=0.75))
    ggplot() %+% dat_hour_sum + aes(x = hour, y = sum, group = variable, color = variable, linetype = variable) + graph_options + palette
  ...
  output$experimentDetails <- renderUI(
    ...
    #sum_experiment <- summarize_df(add_phase(resultInput()[[6]]), "phase", sum_func)
